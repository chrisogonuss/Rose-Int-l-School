 ...
 */
    $featured_gallery_section_title           = galaxy_preschool_get_option( 'featured_gallery_section_title' );
    $featured_gallery_content_type            = galaxy_preschool_get_option( 'featured_gallery_content_type' );
    $number_of_featured_gallery_items         = galaxy_preschool_get_option( 'number_of_featured_gallery_items' );
    $featured_gallery_column                  = galaxy_preschool_get_option( 'featured_gallery_column' );

    if( $featured_gallery_content_type == 'featured_gallery_page' ) :
        for( $i=1; $i<=$number_of_featured_gallery_items; $i++ ) :
            $featured_gallery_posts[] = absint( galaxy_preschool_get_option( 'featured_gallery_page_'.$i ) );
        endfor;  
    elseif( $featured_gallery_content_type == 'featured_gallery_post' ) :
        for( $i=1; $i<=$number_of_featured_gallery_items; $i++ ) :
            $featured_gallery_posts[] = absint( galaxy_preschool_get_option( 'featured_gallery_post_'.$i ) );
        endfor;
    endif;
    ?>
